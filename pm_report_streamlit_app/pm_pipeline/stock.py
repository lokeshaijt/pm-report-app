...
def parse_stock(file_obj) -> dict:
    ...
    for sheet_name in wb.sheetnames:
        ...
        if kind == "raw":
            ...
            return dict(stock)  # raw dump found and used - authoritative, stop here
        ...
        if kind == "pivot":
            ...
            try:
                ...
                grand_total_col = len(headers)  # fall back to last column
            for row in ws.iter_rows(min_row=header_row + 1, values_only=True):
                name = row[0]
                if not name or str(name).strip().lower() == "grand total":
                    continue
                stock[norm(name)] = row[grand_total_col - 1] or 0
            # keep scanning other sheets too, in case a raw dump appears later
            # and should take priority - but if nothing raw is found, this pivot stands.
